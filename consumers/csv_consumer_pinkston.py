...
def process_message(message: str, rolling_window: deque, window_size: int) -> None:
    ...
    try:
        ...
        # Special Alert if high_temp is True
        if high_temp:
            print(f"ALERT! High temperature detected at {timestamp}: {temperature}\u00B0F")
            logger.warning(f"ALERT! High temperature detected at {timestamp}: {temperature}\u00B0F")
